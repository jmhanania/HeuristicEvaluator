...
(function () {
  ...
  shadow.innerHTML = [
    ...
    '  #panel{width:300px;background:#0f172a;color:#e2e8f0;border-radius:0 0 0 12px;',
    '    box-shadow:-4px 4px 24px rgba(0,0,0,.5);overflow:hidden;}',
    '  #hdr{display:flex;align-items:center;justify-content:space-between;',
    '    padding:10px 14px;background:#1e293b;user-select:none;}',
    '  #hdr h1{font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:.06em;text-transform:uppercase;}',
    '  #close{background:none;border:none;color:#475569;cursor:pointer;font-size:20px;line-height:1;padding:2px 0;}',
    '  #close:hover{color:#e2e8f0;}',
    '  #body{padding:14px;display:flex;flex-direction:column;gap:10px;}',
    '  .lbl{font-size:10px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.08em;',
    '    display:block;margin-bottom:3px;}',
    '  select,input[type=text]{width:100%;background:#1e293b;border:1px solid #334155;',
    '    color:#e2e8f0;border-radius:6px;padding:7px 9px;font-size:12px;outline:none;}',
    '  select:focus,input[type=text]:focus{border-color:#3b82f6;}',
    '  #new-flow-row{display:none;margin-top:4px;}',
    '  .btn-primary{width:100%;padding:9px;background:#3b82f6;color:#fff;border:none;',
    '    border-radius:6px;font-size:12px;font-weight:700;cursor:pointer;}',
    '  .btn-primary:hover:not(:disabled){background:#2563eb;}',
    '  .btn-primary:disabled{opacity:.6;cursor:not-allowed;}',
    '  .btn-secondary{width:100%;padding:7px;background:transparent;color:#f59e0b;',
    '    border:1px solid #f59e0b;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer;}',
    '  .btn-secondary:hover{background:#f59e0b22;}',
    '  .btn-secondary.active{background:#f59e0b;color:#000;}',
    '  #status{font-size:11px;color:#475569;min-height:14px;text-align:center;}',
    '  #status.err{color:#f87171;} #status.ok{color:#4ade80;}',
    '  #results{display:none;border-top:1px solid #1e293b;padding-top:10px;}',
    '  .row{display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px;}',
    '  .row .k{color:#475569;} .row .v{font-weight:700;color:#e2e8f0;}',
    '  .v.warn{color:#f59e0b;} .v.good{color:#4ade80;}',
    '</style>',
    '<div id="panel">',
    '  <div id="hdr"><h1>HeuristicEvaluator</h1><button id="close">&times;</button></div>',
    '  <div id="body">',
    '    <div>',
    '      <label class="lbl" for="session-sel">Session</label>',
    '      <select id="session-sel"><option value="">Loading...</option></select>',
    '    </div>',
    '    <div>',
    '      <label class="lbl" for="flow-sel">Flow</label>',
    '      <select id="flow-sel"><option value="">— select session first —</option></select>',
    '      <div id="new-flow-row">',
    '        <input type="text" id="new-flow-name" placeholder="New flow name..." />',
    '      </div>',
    '    </div>',
    '    <div>',
    '      <label class="lbl" for="step-name">Step Name</label>',
    '      <input type="text" id="step-name" placeholder="e.g. Shipping Address Entry" />',
    '    </div>',
    '    <button class="btn-secondary" id="redact-btn">&#9646; Redact Sensitive Content</button>',
